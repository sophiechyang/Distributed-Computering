	...
	if($_SERVER['REQUEST_METHOD'] == 'POST')
	{
		...
		if(is_numeric($accountid) && is_numeric($amount) && intval($amount) > 0)
		{
			...
			else
			{
				...
				if($result)
				{
					echo '<script>alert("Deposit successful.");window.location.href="/bank.html";</script>';
				}
			}
		}
		else
		{
			echo '<script>alert("Bank_Server: Problem with D(eposit.");window.location.href="/bank.html";</script>';
		}
	}
?>
...
<html>
	...
	<body>
		<div>
			<form method="post">
				<fieldset>
					<legend>Deposit (into account) command</legend>
